...
def pprint_body (body, level):
    for statement in body:
        yield '    '*level
        yield from pprint_inner (statement, level)
        yield '\n'
...
def pprint_seq (seq, sep=', '):
    for index, elem in enumerate (seq):
        if type (elem)==str:
            ...
            yield from pprint_inner (elem)

        if index<len (seq)-1:
            if type (sep)==str:
                yield sep
            else:
                yield from pprint_inner (sep)

def pprint_orelse (orelse, level):
    if len (orelse)>0:
        yield '    '*level+'else:\n'
        yield from pprint_body (orelse, level+1)
...
def pprint_args (args, defaults):
    ...
    for index, arg in enumerate (args):
        ...
        if index>=d_index:
            yield '='
            yield from pprint_inner (defaults[index-d_index])
...
def pprint_inner (node, level=0):
    ...
    if t==Add:
        ...
        yield 'assert '
        yield from pprint_inner (node.test)
        # TODO: msg

    elif t==Assign:
        # Assign(targets=[Name(id='c', ctx=Store())],
        #        value=...)
        yield from pprint_seq (node.targets)
        yield '= '
        yield from pprint_inner (node.value)

    elif t==AsyncFor:
        yield 'async '
        # For(target=..., iter=..., body=[...], orelse=[...])
        node= For (target=node.target, iter=node.iter, body=node.body, orelse=node.orelse)
        yield from pprint_inner (node)
        ...
        yield from pprint_inner (node.target)
        yield from pprint_inner (node.op)
        yield '= '
        yield from pprint_inner (node.value)
        ...
        # 5*3+4
        yield from pprint_inner (node.left)
        yield from pprint_inner (node.op)
        yield from pprint_inner (node.right)
        ...
        # TODO: annotations
        yield from pprint_inner (node.func)
        yield ' ('
        yield from pprint_seq (node.args)
        ...
        yield from pprint_seq (node.keywords)
        ...
        if node.starargs is not None:
            yield '*'
            yield from pprint_inner (node.starargs)
        ...
        if node.kwargs is not None:
            yield '**'
            yield from pprint_inner (node.kwargs)
        ...
        if len (node.bases)>0:
            yield ' ('
            yield from pprint_seq (node.bases)
            yield ')'
        ...
        yield from pprint_inner (node.left)
        for op in node.ops:
            yield from pprint_inner (op)

        for comparator in node.comparators:
            yield from pprint_inner (comparator)
        ...
        for k, v in zip (node.keys, node.values):
            yield from pprint_inner (k)
            yield '='
            yield from pprint_inner (v)
            yield ', '
        yield ' }'
        ...
        yield '{ '
        yield from pprint_inner (node.key)
        yield ': '
        yield from pprint_inner (node.value)
        yield ' for '
        # TODO: more
        yield from pprint_inner (node.generators[0])
        yield ' }'
        ...
        if node.type is not None:
            yield from pprint_inner (node.type)
        ...
        # Expr(value=...)
        yield from pprint_inner (node.value, level)
        ...
        yield from pprint_inner (node.target)
        yield ' in '
        yield from pprint_inner (node.iter)
        yield ':\n'
        yield from pprint_body (node.body, level+1)
        yield from pprint_orelse (node.orelse, level)
        ...
        yield '( '
        yield from pprint_inner (node.elt)
        yield ' for '
        # TODO: more
        yield from pprint_inner (node.generators[0])
        yield ' )'
        ...
        yield 'global '
        yield from pprint_seq (node.names)
        ...
        if len (node.orelse)>0:
            ...
            if len (node.orelse)==1 and type (node.orelse[0])==If:
                yield '    '*level+'el'
                yield from pprint_inner (node.orelse[0], level)
            else:
                yield from pprint_orelse (node.orelse, level)

    elif t==IfExp:
        # IfExp(test=..., body=Str(s=''), orelse=Str(s='s'))
        yield from pprint_inner (node.body)
        yield ' if '
        yield from pprint_inner (node.test)
        yield ' else '
        yield from pprint_inner (node.orelse)
        ...
        yield '[ '
        yield from pprint_seq (node.elts)
        yield ' ]'
        ...
        # Module(body=[ ... ])
        yield from pprint_body (node.body, 0)
        ...
        if node.exc is not None:
            yield from pprint_inner (node.exc)
        # TODO: cause?
        ...
        if node.lower is not None:
            yield from pprint_inner (node.lower)

        yield ':'

        if node.upper is not None:
            yield from pprint_inner (node.upper)

        if node.step is not None:
            yield ':'
            yield from pprint_inner (node.step)
        ...
        yield from pprint_inner (node.value)
        yield '['
        yield from pprint_inner (node.slice)
        yield ']'
        ...
        if len (node.finalbody)>0:
            yield '    '*level+'finally:\n'
            yield from pprint_body (node.finalbody, level+1)
        ...
        yield from pprint_inner (node.op)
        yield from pprint_inner (node.operand)
        ...
        yield 'with '
        yield from pprint_seq (node.items)
        yield ':\n'
        yield from pprint_body (node.body, level+1)
        ...
        yield from pprint_args (node.args, node.defaults)
        ...
        if node.vararg is not None:
            yield '*'
            yield from pprint_inner (node.vararg)
        ...
        yield from pprint_args (node.kwonlyargs, node.kw_defaults)
        ...
        if len (node.ifs)>0:
            # TODO: more
            yield ' if '
            yield from pprint_inner (node.ifs[0])
        ...
        yield from pprint_inner (node.context_expr)
        if node.optional_vars is not None:
            yield ' as '
            yield from pprint_inner (node.optional_vars)
